...
class RafahCrossingController extends Controller
{

    public function archive_rafah(Request $request)
    {
        if ($request->ajax()) {

            $data = MediaCenter::query();

            $data = $data->select('*')->onlyTrashed()->where('type', 'rafah')->orderBy('id', 'desc');
        ...
        return view('admin.news.rafah.archive_rafah');
    }
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = $data->select('*')->where('type', 'rafah')->orderBy('id', 'desc');
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $btn .= '<a href=" ' . route("rafahCrossing.edit", $row->id) . '"><i class="text-info fas fa-edit" style="font-size: 16px"></i>&nbsp;&nbsp;</a>';
        ...
        return view('admin.news.rafah.index');
    }
    ...
    {
        return view('admin.news.rafah.add');
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            $pathbasicimage = $file->storeAs('files/news/rafah', $basicFileName, ['disk' => 'public']);
            ...
            if ($request->file('files')) {
                ...
                foreach ($request->file('files') as $file) {
                    ...
                    $path_file = $file->storeAs('files/news/rafah', $fileName, ['disk' => 'public']);
                    $store->files()->create([
                        "filename" => $path_file,
                    ]);
                }
    ...
    {
        $news = MediaCenter::findOrFail($id);
        return view('admin.news.rafah.edit', compact('news'));
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        try {
            ...
            if ($file = $request->file('basicFile')) {
                ...
                $basicFileName = time() . $file->getClientOriginalName();
                $pathImage = $file->storeAs('files/news/rafah', $basicFileName, ['disk' => 'public']);
            }
            ...
            if ($request->file('files')) {
                ...
                foreach ($request->file('files') as $file) {
                    $fileName = time() . $file->getClientOriginalName();
                    $path = $file->storeAs('files/news/rafah', $fileName, ['disk' => 'public']);
