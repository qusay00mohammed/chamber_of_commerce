...
class ErezCrossingController extends Controller
{

    public function archive_erez(Request $request)
    {
        if ($request->ajax()) {

            $data = MediaCenter::query();

            $data = $data->select('*')->onlyTrashed()->where('type', 'erez')->orderBy('id', 'desc');
        ...
        return view('admin.news.erez.archive_erez');
    }
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = $data->select('*')->where('type', 'erez')->orderBy('id', 'desc');
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $btn .= '<a href=" ' . route("erezCrossing.edit", $row->id) . '"><i class="text-info fas fa-edit" style="font-size: 16px"></i>&nbsp;&nbsp;</a>';
        ...
        return view('admin.news.erez.index');
    }
    ...
    {
        return view('admin.news.erez.add');
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            $pathbasicimage = $file->storeAs('files/news/erez', $basicFileName, ['disk' => 'public']);
            ...
            if ($request->file('files')) {
                ...
                foreach ($request->file('files') as $file) {
                    ...
                    $path_file = $file->storeAs('files/news/erez', $fileName, ['disk' => 'public']);
                    $store->files()->create([
                        "filename" => $path_file,
                    ]);
                }
    ...
    {
        $news = MediaCenter::findOrFail($id);
        return view('admin.news.erez.edit', compact('news'));
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        try {
            ...
            if ($file = $request->file('basicFile')) {
                ...
                $basicFileName = time() . $file->getClientOriginalName();
                $pathImage = $file->storeAs('files/news/erez', $basicFileName, ['disk' => 'public']);
            }
            ...
            if ($request->file('files')) {
                ...
                foreach ($request->file('files') as $file) {
                    $fileName = time() . $file->getClientOriginalName();
                    $path = $file->storeAs('files/news/erez', $fileName, ['disk' => 'public']);
