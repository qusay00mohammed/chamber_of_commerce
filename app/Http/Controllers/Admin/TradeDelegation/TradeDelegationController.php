...
namespace App\Http\Controllers\Admin\TradeDelegation;
...
class TradeDelegationController extends Controller
{
    /**
    ...
    {
        if($request->ajax()) {
            $data = TradeDelegation::orderBy('id', 'desc')->select('*');
        ...
        return view('admin.trade-delegations.index');
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            $path = $image->storeAs('files/trade-delegations', $fileName, ['disk' => 'public']);
            // end upload file

            $store = TradeDelegation::create([
                "title" => $request->title,
                "image_url" => $path,
            ]);
    ...
    {
        try {
            $partner = TradeDelegation::findOrFail($request->partner_id);
            ...
            if($file = $request->file('image')) {
                ...
                $path = $file->storeAs('files/trade-delegations', $fileName, 'public');
            }
            // end upload file
            ...
            return redirect()->route('tradeDelegations.index');
        } catch(\Exception $e) {
    ...
    {
        $delete = TradeDelegation::findOrFail($id);
