...
class ActivityController extends Controller
{
    ...
    public function archive_activity(Request $request)
    {
        if ($request->ajax()) {

            $data = MediaCenter::query();

            $data = $data->select('*')->onlyTrashed()->where('type', 'activity')->orderBy('id', 'desc');
            ...
            return DataTables::of($data)
                ...
                ->addColumn('image', function ($row) {
                            ...
                            $url = asset("storage/files/activities/$row->basicFile");
        ...
        return view('admin.activities.archive_activity');
    }
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = $data->select('*')->where('type', 'activity')->orderBy('id', 'desc');
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $btn .= '<a href=" ' . route("activities.edit", $row->id) . '"><i class="text-info fas fa-edit" style="font-size: 16px"></i>&nbsp;&nbsp;</a>';
    ...
    public function store(Request $request)
    {
        try {
            ...
            $file->storeAs('files/activities', $basicFileName, ['disk' => 'public']);
            ...
            if ($request->file('files')) {
                ...
                foreach ($request->file('files') as $file) {
                    // foreach ($file as $f) {
                        ...
                        $file->storeAs('files/activities', $fileName, ['disk' => 'public']);
                        $store->files()->create([
                            "filename" => $fileName,
                        ]);
                    // }
    ...
    public function destroy(Request $request, string $id)
    {
        try {
            ...
            //     Storage::disk('public')->delete("files/activities/$fileName");
            //     $image->delete();
            // }
            // Storage::disk('public')->delete("files/activities/$delete->basicFile");
            $delete->delete();
