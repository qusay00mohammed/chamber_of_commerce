...
@push('javascript')
    ...
    <script>
        var map1;
        var infoWindow;
        var currentMarker; // متغير لتتبع العلامة الحالية
        ...
        // إنشاء مربع نص لعرض المعلومات
        infoWindow = new google.maps.InfoWindow();


        // var latitude = {{ $committee->latitude }}; // استبدل بقيمة خط العرض الخاصة بك
        // var longitude = {{ $committee->longitude }}; // استبدل بقيمة خط الطول الخاصة بك

        // var location = new google.maps.LatLng(latitude, longitude);
        // // إنشاء علامة على الموقع
        // var marker = new google.maps.Marker({
        //     position: location,
        //     map: map,
        // });

        // infoWindow.setContent("الموقع: " + latitude + ", " + longitude);
        ...
        map1.addListener("click", function(e) {
            if (currentMarker) {
                // إذا كان هناك علامة حالية، قم بإزالتها
        ...
        function performStore() {
            ...
            updateURL = "{{ route('committees.update', [$committee->id]) }}";
            redirectUrl = "{{ route('committees.index') }}";
...
@section('toolbar')
    <h1 class="d-flex align-items-center text-dark fw-bolder fs-3 my-1">الغرفة التجارية
        ...
        <span class="h-20px border-gray-200 border-start ms-3 mx-2"></span>
        <small class="text-muted fs-7 fw-bold my-1 ms-1">تعديل لجنة</small>
    </h1>
@endsection
                            ...
                            <div class="col-lg-6">
                                <div class="form-group mg-b-0">
                                    ...
                                    <input class="form-control mb-2" name="phone_number" required type="text"
                                        placeholder="رقم الجوال" value="{{ $committee->phone_number }}">
                                </div>
                            </div>
                        ...
                        <div class="row row-sm">
                            <div class="col-lg-6">
                                ...
                                <input class="form-control mb-2" id="longitude" disabled
                                    value="{{ $committee->longitude }}" name="longitude" required type="hidden"
                            ...
                            <div class="col-lg-6">
                                <div class="form-group mg-b-0">
                                    ...
                                    <input class="form-control mb-2" id="latitude" disabled
                                        value="{{ $committee->latitude }}" name="latitude" required type="hidden"
                                    ...
                                    <optgroup label="اعضاء مجلس الإدارة">
                                        @foreach ($members as $item)
                                            @if ($item->type == 'council')
                                                @foreach ($committee->members as $i)
                                                    @if ($item->id == $i->id)
                                                        <?php $id = $item->id; ?>
