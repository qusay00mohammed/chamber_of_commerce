...
@section('title', 'تعديل عضو مجلس إدارة')
...
@push('javascript')
    ...
    <script>
        ...
        function performUpdate() {
            ...
            updateURL = "{{ route('administrationMembers.update', [$member->id]) }}";
            redirectUrl = "{{ route('administrationMembers.index') }}";
...
@section('toolbar')
    <h1 class="d-flex align-items-center text-dark fw-bolder fs-3 my-1">الغرفة التجارية
        ...
        <span class="h-20px border-gray-200 border-start ms-3 mx-2"></span>
        <small class="text-muted fs-7 fw-bold my-1 ms-1">تعديل عضو مجلس إدارة</small>
    </h1>
@endsection
                    ...
                    <form id="create_form" onsubmit="event.preventDefault(); performUpdate();">
                        ...
                        <div class="row row-sm">
                            ...
                            <input class="form-control mb-2" name="fullname" required type="text"
                                placeholder="اسم العضو الكامل" value="{{ $member->full_name }}">
                            ...
                            <div class="col-lg-6">
                                <div class="form-group mg-b-0">
                                    ...
                                    <input class="form-control mb-2" name="job_title" required type="text"
                                        placeholder="المسمى الوظيفي" value="{{ $member->job_title }}">
                            ...
                            <div class="col-lg-6">
                                <div class="form-group mg-b-0">
                                    ...
                                    <select class="form-select form-select" name="administration" id="administration" required
                                        ...
                                        <option value="1" {{ $member->administration == 1 ? 'selected' : '' }}>نعم</option>
                                        <option value="0" {{ $member->administration == 0 ? 'selected' : '' }}>لا</option>
                                    </select>
                                </div>
                            </div>

                            <label for="" class="mb-3">إضافة وصف</label>
                            <textarea id="editor" name="description" class="tox-target" placeholder="الوصف يكون فقط لرئيس مجلس الادارة فقط">{{ $member->description }}</textarea>
                        </div>
