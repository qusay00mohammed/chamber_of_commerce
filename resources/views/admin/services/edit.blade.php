    ...
    <script>
        ...
        function performStore() {
            ...
            for (var i = 0; i < icon_image.length; i++) {
                data.append('icon_image[]', icon_image[i]); // Append each file to the FormData object
            }

            var background_image = myDropzone1.getAcceptedFiles(); // Get the selected files
            for (var i = 0; i < background_image.length; i++) {
                data.append('background_image[]', background_image[i]); // Append each file to the FormData object
            }

            updateURL = "{{ route('services.update', [$service->id]) }}";
...
@section('toolbar')
    <h1 class="d-flex align-items-center text-dark fw-bolder fs-3 my-1">الغرفة التجارية
        ...
        <span class="h-20px border-gray-200 border-start ms-3 mx-2"></span>
        <small class="text-muted fs-7 fw-bold my-1 ms-1">تعديل خدمة</small>
    </h1>
@endsection
                ...
                <div class="card-body">
                    <form id="create_form" onsubmit="event.preventDefault(); performStore();">
                        ...
                        <div class="row row-sm">
                            ...
                            <label for="" class="mb-3">عنوان الخدمة</label>
                            <input class="form-control mb-2" name="title" required type="text"
                                placeholder="عنوان الخدمة" value="{{ $service->title }}">

                            <label for="" class="mb-3">وصف الخدمة</label>
                            <textarea id="editor" name="description" class="tox-target" placeholder="تفاصيل الخدمة">{{ $service->description }}</textarea>
                        </div>
                        ...
                        <div class="row row-sm">
                            <div class="col-lg-6">
                                ...
                                <h5 class="card-title">صورة الخلفية</h5>
